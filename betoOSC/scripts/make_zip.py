...
def  main():
  ...
  installer = "\installer\betoOSC Installer.exe"
   
  if demo:
    installer = "\installer\betoOSC Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\betoOSC manual.pdf" 
  ]
  ...
  ZIPNAME = "betoOSC-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "betoOSC-v" + FULLVERSIONSTR + "-win-demo.zip"
