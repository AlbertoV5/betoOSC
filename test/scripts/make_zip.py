...
def  main():
  ...
  installer = "\installer\test Installer.exe"
   
  if demo:
    installer = "\installer\test Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\test manual.pdf" 
  ]
  ...
  ZIPNAME = "test-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "test-v" + FULLVERSIONSTR + "-win-demo.zip"
